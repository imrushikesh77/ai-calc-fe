...
function DrawingCanvas({ onSubmit, isLoading }) {
  ...
  useEffect(() => {
    if (contextRef.current) {
      contextRef.current.strokeStyle = activeTool === TOOLS.ERASER ? "#FFFFFF" : color
      ...
      setCurrentPath([
        {
          x: offsetX,
          y: offsetY,
          color: activeTool === TOOLS.ERASER ? "#FFFFFF" : color,
          brushSize,
          tool: activeTool,
        },
      ])
  ...
  const redrawCanvas = useCallback(() => {
    ...
    context.strokeStyle = activeTool === TOOLS.ERASER ? "#FFFFFF" : color
    context.lineWidth = brushSize
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="border rounded-lg overflow-hidden bg-white relative" style={{ touchAction: "none" }}>
        <canvas
          ref={canvasRef}
